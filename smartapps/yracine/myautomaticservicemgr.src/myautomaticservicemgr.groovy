...
def authPage() {
	...
	log.debug "authPage>redirectUrl=${redirectUrl},atomicState.authToken=${atomicState.authToken},atomicState.oauthTokenProvided=${atomicState?.oauthTokenProvided}"
	...
	try {
		httpGet(deviceListParams) { resp ->
			...
			if (resp.status == AUTOMATIC_SUCCESS) {
				...
				msg= "trying to get list of Automatic connected vehicles, http error status: ${resp.status}"
				log.error msg      
				send "MyAutomaticServiceMgr>${msg}"                
			}
        
		}        
	} catch (java.net.UnknownHostException e) {
		msg= "trying to get list of Automatic Devices, Unknown host - check the URL " + deviceListParams.uri
		log.error msg      
		send "MyAutomaticServiceMgr>${msg}"                
		        
	} catch (java.net.NoRouteToHostException t) {
		msg= "trying to get list of Automatic Devices, No route to host - check the URL " + deviceListParams.uri
		log.error msg        
		send "MyAutomaticServiceMgr>${msg}"                
	} catch (e) {
		msg= "exception $e while getting list of Automatic Devices" 
		log.error msg        
		send "MyAutomaticServiceMgr>${msg}"                
    }
...
def initialize() {
	...
	if ((delay < 5) || (delay>59)) {
		msg= "MyAutomaticServiceMgr>scheduling interval not in range (${delay} min), exiting..."
		log.debug msg
		send msg
		return
	}
...
def takeAction() {
	...
	def devices = AutomaticDevices.collect { dni ->
		...
		try {
			...
			atomicState?.exceptionCount=atomaticState?.exceptionCount+1        
			log.error "MyAutomaticServiceMgr>exception $e while trying to poll the device $d, exceptionCount= ${atomicState?.exceptionCount}" 
		}
		if ((atomaticState?.exceptionCount>=MAX_EXCEPTION_COUNT) || ((exceptionCheck) && (exceptionCheck.contains("Unauthorized")))) {
			// need to re-authenticate again    
			atomicState.authToken= null                    
			atomicState?.oauthTokenProvided = false
			msg = "MyAutomaticServiceMgr>too many exceptions/errors or unauthorized exception, $exceptionCheck (${atomicState?.exceptionCount} errors), need to re-authenticate at Automatic..." 
			log.error msg
			send msg
...
def procEvent() {
	...
	try {
		eventFields = new JsonSlurper().parseText(request.JSON.toString())   
		log.trace "procEvent>eventFields = $eventFields"
	} catch (e) {
	...
	if (eventFields) {
		def vehicle=eventFields.vehicle        
		vehicleId=vehicle.id.toString() 
		log.debug "procEvent>vehicleId = $vehicleId"
		def devices = AutomaticDevices.collect { dni ->
    
			def d = getChildDevice(dni)
			log.debug "takeAction>looping thru Automatic Devices, found id $dni, about to compare vehicleIds"
			if (d) {
				if (dni.contains(vehicleId)) {
					log.debug "procEvent>found vehicleId=$vehicleId"
					d.generateVehicleRTEvents("", eventFields)
					foundVehicle=true                    
				}                    
			}                    
		}
...
def oauthInitUrl() {
	...
	def oauthParams = [
		...
		scope: "scope:public scope:user:profile scope:location scope:vehicle:profile " +
        	"scope:vehicle:events scope:trip scope:behavior", // to be added: scope:current_location", for RT events with websockets 
		client_id: stcid,
...
def getSmartThingsClientId() { "97ade3d3cfaf2031c5fc" }

def getSmartThingsPrivateKey() { "44535ad34672c3c56abd0148ae2de38a96f52497" }